 ...
 */
function EntryDetail({ entry, onClose, relatedEntries = [], onTagClick }) {
  ...
  return (
    <div
      ref={detailRef}
      tabIndex={-1}
      className="h-full overflow-y-auto p-6 font-mono fade-in outline-none"
    >
      {/* Header */}
      <div className="mb-6">
        ...
        <h2 className="text-fg text-lg leading-relaxed font-semibold">
          {entry.content}
        </h2>
      </div>
      ...
      {/* Braille Art */}
      <div className="mb-6">
        <div className="text-dim text-xs mb-2">fingerprint:</div>
      ...
      {entry.commentary && (
        <div className="mb-6">
          <div className="text-dim text-xs mb-2">commentary:</div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {relatedEntries.slice(0, 5).map((related) => (
              <div key={related.id} className="flex items-start gap-2 text-sm">
